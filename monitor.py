...
# Market regime — SPY vs. its own 200-day MA, used to gate the experimental
# weighted signal's bearish calls (see compute_weighted_signal). Needs much
# more history than the 5-day quotes fetch_market_overview uses, so it's
# fetched separately.
...
def compute_spy_regime(closes):
    """SPY price vs. its own 200-day MA — the broad market regime that gates
    the experimental weighted signal's bearish calls (see
    compute_weighted_signal). Backtesting showed bearish signals only had
    real skill in 2022, the one confirmed downtrend year in the 5-year
    sample; every other year they underperformed a coin flip. Defaults to
    "uptrend" (the conservative choice — bearish stays suppressed) when
    there isn't enough history to judge."""
    if len(closes) < SPY_REGIME_MA_PERIOD:
        return {"label": "uptrend", "pct_vs_ma": None}
    ma = float(closes.rolling(SPY_REGIME_MA_PERIOD).mean().iloc[-1])
    current = float(closes.iloc[-1])
    if ma == 0:
        return {"label": "uptrend", "pct_vs_ma": None}
    pct_vs_ma = (current - ma) / ma * 100
    label = "downtrend" if pct_vs_ma < 0 else "uptrend"
    return {"label": label, "pct_vs_ma": pct_vs_ma}
...
def fetch_regime_context():
    """Single-ticker SPY history fetch, independent of fetch_market_overview's
    batched 5-day quotes since the 200-day MA regime read needs much more
    history. Falls back to "uptrend" on any fetch problem, same default as
    compute_spy_regime."""
    try:
        hist = yf.download(
            SPY_REGIME_TICKER, period=SPY_REGIME_HISTORY_PERIOD, interval="1d",
        ...
        print(f"[warn] regime context fetch failed: {exc}", file=sys.stderr)
        return {"label": "uptrend", "pct_vs_ma": None}
    return compute_spy_regime(closes)
...
# Maps each factor to the "dimension" it belongs to and reads its lean back
# out of a built factors list — shared by backtest.py's factor-breakdown
# reporting and fit_weights.py's regression so both interpret build_factors'
# output the same way monitor.py itself does. RSI and volume labels embed
# exact numbers ("RSI 71", "Vol: 1.6x avg"), so these match by prefix rather
# than exact text; MA50 and breadth labels are fixed, discrete strings.
FACTOR_DIMENSIONS = {
...
# Experimental, parallel signal: each factor (and the SPY regime) contributes
# its own fitted log-odds weight instead of compute_signal's equal ±1 vote.
# Generated by fit_weights.py — logistic regression predicting whether price
# rose over the following ACCURACY_RESOLUTION_DAYS days, fit on
# backtest_results/all_days.json (every trading day in the 5-year/104-ticker
# backtest, not just days the tally already called directional — training
# only on tally-flagged days would bake the tally's own selection bias into
# these weights). Regenerate by re-running `python backtest.py` then
# `python fit_weights.py` and pasting in the printed dict; this is a plain
# committed constant, not something loaded at runtime.
#
# Regime was tried first as a hardcoded "suppress LEANS NEGATIVE outside a
# confirmed downtrend" rule — backtesting that directly showed the opposite
# of what was assumed (bearish signals were *less* reliable, not more, when
# SPY itself was trending down; see report.md's "By market regime" table).
# It's included here as just another weighted dimension instead, so its
# real (and non-obvious) effect gets sized by the regression rather than
# hand-picked.
#
# Notable finding baked into these particular numbers: every weight is
# positive except ma50's (-0.0134), and the intercept alone (+0.0856,
# the 5-year sample's base rate of price rising over the window) already
# exceeds the largest possible negative contribution. That means, as
# currently fit, no combination of these four factors produces a net
# negative score — LEANS NEGATIVE is expected to be rare to nonexistent
# from this function. That's not a bug: it's the same thing report.md
# already showed (LEANS NEGATIVE's raw win rate was 45.9%, worse than a
# coin flip) confirmed from a different angle — within this factor set,
# there's no real recipe for betting on a decline, oversold-bounce
# dynamics dominate whatever else is present. See README for more.
...
def compute_weighted_signal(factors, regime):
    """Runs alongside compute_signal, not in place of it, until walk-forward
    testing validates it (see README) — an experimental second read using
    WEIGHTED_FACTOR_WEIGHTS instead of an equal-weight tally.

    `score` is a raw log-odds of price rising over the window — every day
    in the 5-year backtest scores positive (range 0.07 to 0.53; sigmoid of
    the minimum is still ~52% "up"), so an absolute score > 0 threshold
    alone makes this an always-on classifier: virtually every day reads
    LEANS POSITIVE, discriminating nothing.

    The fix is NOT to relabel weaker-than-average days LEANS NEGATIVE —
    that was tried and rejected: a day scoring 0.07 still implies ~52% odds
    of rising, so calling it "negative" would overstate the signal in the
    opposite direction. Instead, LEANS POSITIVE is reserved for scores
    *meaningfully above* the intercept (the baseline drift every day
    shares, regardless of that day's factors); at-or-below-baseline days
    fall to MIXED SIGNALS instead, which — unlike LEANS NEGATIVE — asserts
    no direction at all, so nothing is overstated. Genuine LEANS NEGATIVE
    stays reserved for an actual score < 0 (currently near-unreachable —
    see the module-level comment above WEIGHTED_FACTOR_WEIGHTS — but
    handled correctly if a future refit ever produces one)."""
    score = WEIGHTED_FACTOR_WEIGHTS["intercept"]
    non_neutral = 0
    for dim, extractor in FACTOR_DIMENSIONS.items():
        lean = extractor(factors)
        if lean is not None and lean != "neutral":
            non_neutral += 1
        if lean is not None:
            score += WEIGHTED_FACTOR_WEIGHTS[dim].get(lean, 0.0)
    score += WEIGHTED_FACTOR_WEIGHTS["regime"].get(regime["label"], 0.0)
    above_baseline = score - WEIGHTED_FACTOR_WEIGHTS["intercept"]

    if non_neutral == 0:
        label, cls, icon = "NOTHING COMPELLING", "mixed", "◆"
    elif score < 0:
        label, cls, icon = "LEANS NEGATIVE", "bearish", "▼"
    elif above_baseline > 0:
        label, cls, icon = "LEANS POSITIVE", "bullish", "▲"
    else:
        label, cls, icon = "MIXED SIGNALS", "mixed", "⚠"
...
def process_ticker(raw, state, today_str, avg_daily_pct, regime):
    ...
    if raw["current_price"] is not None:
        ...
        weighted_signal = compute_weighted_signal(factors, regime)
...
def build_dashboard(entries, generated_at, notable_movers=None, market_overview=None, regime=None):
...
def main():
    ...
    regime = fetch_regime_context()

    email_alerts = []
    dashboard_entries = []
    for raw in raw_data:
        email_result, should_email, dashboard_entry = process_ticker(raw, state, today_str, avg_daily_pct, regime)
        if should_email:
    ...
    save_dashboard(
        DASHBOARD_DATA_FILE,
        build_dashboard(dashboard_entries, generated_at, notable_movers, market_overview, regime),
    )
